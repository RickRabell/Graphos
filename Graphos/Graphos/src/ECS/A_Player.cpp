...
static float GetTotalTimeSeconds() {
...
float A_Player::getTotalTime() const { return m_totalTime; }
int A_Player::getLastWaypointPassed() const { return m_lastWaypointPassed; }
float A_Player::getDistToNextWaypoint() const {
    if (m_waypoints.empty()) return 0.f;
    int nextIdx = (m_lastWaypointPassed + 1) % m_waypoints.size();
    auto transform = getComponent<Transform>(); // Ahora usará la versión const
    if (!transform) return 0.f;
    return Distance(transform->getPosition(), m_waypoints[nextIdx]);
}
bool A_Player::isFrozen() const { return m_frozen; }
void A_Player::freeze() { m_frozen = true; moveSpeed = 0.f; }