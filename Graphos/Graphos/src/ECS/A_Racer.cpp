...
void A_Racer::seekToWaypoint(float deltaTime) {
    ...
    if (dist <= m_arrivalThreshold * 2.0f && dist > m_arrivalThreshold) {
        ...
        std::uniform_real_distribution<float> approachSpeedDist(m_minApproachSpeed, m_maxApproachSpeed);
    ...
    if (dist <= m_arrivalThreshold) {
        ...
        static std::random_device rd;
        static std::mt19937 gen(rd());
        std::uniform_real_distribution<float> waypointSpeedDist(m_minWaypointSpeed, m_maxWaypointSpeed);
        m_speed = waypointSpeedDist(gen);

        // Solo cuenta la vuelta si venía del último waypoint al primero
        if (m_currentWaypointIndex == 0 && prevWaypoint == static_cast<int>(m_waypoints.size()) - 1) {
            if (m_hasStartedLap) {
                m_laps++;
